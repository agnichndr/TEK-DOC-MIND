alter table public.project_llm_connectors
  add column credential_ciphertext text,
  add column credential_nonce text,
  add column credential_auth_tag text,
  add column credential_key_version smallint,
  add constraint project_llm_connectors_encrypted_credential_complete check (
    (
    ...
    or (
      char_length(credential_ciphertext) between 1 and 65536
      and char_length(credential_nonce) between 1 and 128
      and char_length(credential_auth_tag) between 1 and 128
      and credential_key_version between 1 and 32767
    )
  );

drop function public.list_project_llm_connectors(text);
drop function public.save_project_llm_connector(text, text, jsonb);
...
as $$
  select
    ...
    connectors.summary,
    connectors.credential_ciphertext,
    connectors.credential_nonce,
    connectors.credential_auth_tag,
    connectors.credential_key_version,
...
grant execute on function public.list_project_llm_connectors(text)

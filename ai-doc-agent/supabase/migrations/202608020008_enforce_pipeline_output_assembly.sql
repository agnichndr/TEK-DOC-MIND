  ...
  loop
    if v_output.output_config ? 'sourceNodeIds' then
      ...
      if jsonb_array_length(v_output.output_config -> 'sourceNodeIds') <> (
        ...
        v_output.output_config -> 'sourceNodeIds'
          @> jsonb_build_array(v_output.id::text)
      ) then
        raise exception 'invalid output source mapping';
      end if;
      ...
      if exists (
        ...
        where sources.id is null
          or (
            sources.id <> v_output.id
            and sources.node_kind <> 'agent'
          )
      ) then
        raise exception 'invalid output source mapping';
      end if;
...
drop trigger if exists enforce_pipeline_output_assembly_on_pipeline
  on public.project_pipelines;
create constraint trigger enforce_pipeline_output_assembly_on_pipeline
after insert or update on public.project_pipelines
deferrable initially deferred
for each row execute function public.enforce_project_pipeline_output_assembly();

drop trigger if exists enforce_pipeline_output_assembly_on_nodes
  on public.project_pipeline_nodes;
create constraint trigger enforce_pipeline_output_assembly_on_nodes
after insert or update or delete on public.project_pipeline_nodes
deferrable initially deferred
for each row execute function public.enforce_project_pipeline_output_assembly();
